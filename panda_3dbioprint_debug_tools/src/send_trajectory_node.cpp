...
int main(int argc, char **argv)
{
  ...
  if (path_type.compare("line") == 0)
    ...
    traj = TaskTrajectoryPlanner::circular_trajectory(initial_pose, center, radius, ttime, freq, loops, path_plane, PLAN_MODE::LSPB);
  else if (path_type.compare("circular_spiral") == 0)
    traj = TaskTrajectoryPlanner::circular_spiral_trajectory(initial_pose, eradius, iradius, ttime, freq, loops, path_plane, PLAN_MODE::LSPB);
  else
  {
  ...
  while ((opt = getopt_long(argc, argv, short_opts, long_opts, nullptr)) != -1)
  {
    switch (opt)
    {
      ...
      default:
        ...
        std::cout << "send_trajectory -smrtf -p <path_type> -P <trajectory_plane> -T <trajectory_time> -L <number_loops> \
