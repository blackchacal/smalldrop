...
#include <smalldrop_msgs/PoseEuler.h>
...
protected:
   ...
   * Class members
   *****************************************************************************************/
  bool torques_to_robot_ =
      true; /** \var Flag the controls if the calculated torques are sent to the robot joints or not. */
  ...
  Eigen::Quaterniond orient_d_target_;     /** \var Desired end-effector orientation quaternion filtered, represented on
                                              base frame. */
  ...
  ros::Publisher pose_euler_pub_;  /** \var Publisher to current pose with euler angles topic. */
  ros::Publisher pose_d_euler_pub_;/** \var Publisher to desired pose with euler angles topic. */
  ...
  /**
   * \fn Eigen::Vector3d R2EulerAngles(Eigen::Matrix3d& Rotation)
   ...
   * \return Returns a 3D vector (double).
   */
  Eigen::Vector3d R2EulerAngles(Eigen::Matrix3d& Rotation);
  ...
  /**
   * \fn Eigen::Matrix<double, 7, 1> distanceFromMechanicalJointLimits(const Eigen::Matrix<double, 7, 1> q, const
   * Eigen::Matrix<double, 7, 1> min_joint_limits, const Eigen::Matrix<double, 7, 1> max_joint_limits) \brief Calculates
   * distance from mechanical joint limits null space objective function.
   *
   ...
   */
  void publishCurrentPose(const Eigen::Vector3d X0ee, Eigen::Matrix3d R0ee);

  /**
   * \fn void publishTorques(const Eigen::VectorXd tau_d, const Eigen::VectorXd tau_task, const Eigen::VectorXd
   * tau_null) \brief Publishes the impedance calculated torques (desired torque, task torque and null space torque) to
   * a topic.
   */
